...
onMounted(async () => {
  try {
    loading.value = true;
    if (!bookId) {
      error.value = "Không tìm thấy ID sách trong URL.";
      return;
    }
    ...
    console.error(err);
    error.value = "Lỗi khi tải dữ liệu sách.";
  } finally {
    loading.value = false;
  }
});
    ...
    <div v-else class="grid grid-cols-12 gap-8">
      <div class="col-span-3 space-y-6">
        ...
        <div class="bg-white border rounded-lg shadow-sm p-4">
          <h3 class="font-semibold mb-2 text-gray-800 text-sm uppercase tracking-wide">
            People You Follow
          </h3>
          <p class="text-gray-500 text-sm italic">
            Nobody you're following has interacted with this book.
          </p>
        </div>
      </div>
      ...
      <div class="col-span-6 space-y-6">
        <div>
          ...
          <p class="text-gray-500 text-sm mt-2">
            ...
            {{ formatYear(book.published_date) }}
          </p>
        </div>
        ...
        <div class="bg-white border rounded-lg shadow-sm p-4">
          <h3 class="font-semibold text-gray-800 mb-2 uppercase text-sm tracking-wide">
            Description
          </h3>
        ...
        <div class="bg-white border rounded-lg shadow-sm p-4">
          ...
          <div class="flex items-center space-x-2">
            <span class="text-2xl font-bold text-teal-600">{{ book.rating }}</span>
            <span class="text-yellow-500">★★★★☆</span>
            <span class="text-gray-500 text-sm">
              based on {{ book.review_count }} reviews
            </span>
          </div>
        </div>
      ...
      <div class="col-span-3 space-y-6">
        ...
        <div class="text-sm text-gray-700 space-y-2">
          <a href="#" class="block text-yellow-600 hover:text-yellow-700">Browse similar books...</a>
          <a href="#" class="block text-yellow-600 hover:text-yellow-700">Start a readalong...</a>
          <a href="#" class="block text-yellow-600 hover:text-yellow-700">Start a buddy read...</a>
          <a href="#" class="block text-yellow-600 hover:text-yellow-700">View question bank...</a>
        </div>

        <div class="border-t border-gray-200 pt-4 text-sm text-gray-600">
          <p><strong>Published:</strong> {{ formatDate(book.published_date) }}</p>
