...
async function loadBuddyRead() {
  try {
    ...
    console.error("Lỗi tải buddy read:", error);
  }
}
...
async function loadMembers() {
  try {
    ...
    console.error("Lỗi tải thành viên buddy read:", error);
  }
}
...
async function postComment() {
  ...
  try {
    ...
    await axios.post(
      `${BOOK_SERVICE_URL}buddyreads/${buddyReadId}/comments`, 
      formData
    );
    newComment.value = "";
    await loadComments(); 
...
async function deleteBuddyRead() {
  if (!confirm("Bạn có chắc muốn XÓA phòng đọc này? Hành động này sẽ xóa phòng đọc của TẤT CẢ thành viên.")) return;
  ...
  try {
    ...
    await axios.delete(
      `${BOOK_SERVICE_URL}buddyreads/${buddyReadId}`,
      { data: formData }
    );
    
    router.push('/community'); 
  } catch (err: any) {
    alert(err.response?.data?.detail || "Lỗi khi xóa phòng");
  } finally {
...
async function leaveBuddyRead() {
  if (!confirm("Bạn có chắc muốn RỜI KHỎI phòng đọc này?")) return;
  ...
  try {
    ...
    await axios.delete(
      `${BOOK_SERVICE_URL}buddyreads/${buddyReadId}/leave`,
      { data: formData }
    );
    
    router.push('/community'); 
  } catch (err: any) {
    alert(err.response?.data?.detail || "Lỗi khi rời phòng");
  } finally {
    isDeleting.value = false;
  }
}
...
async function searchUsers() {
  ...
  try {
    ...
    searchResults.value = res.data.filter(
      (user: SearchUser) => user.id !== currentUserId && !memberIds.includes(user.id)
    );
  } catch (err) {
...
async function sendInvite(inviteeId: number) {
  ...
  try {
    ...
    await axios.post(
      `${BOOK_SERVICE_URL}buddyreads/${buddyReadId}/invite`,
      formData
    );
...
<template>
  ...
  <div class="max-w-3xl mx-auto p-4 sm:p-6 mt-8">
    ...
    <div v-else-if="!buddyRead" class="text-center text-red-500 py-10">
      Không tìm thấy phòng đọc này.
    </div>

    <div v-else>
      <button @click="router.push('/community')" 
        class="text-sm font-medium text-yellow-600 hover:text-yellow-800 mb-4">
        &larr; Quay lại Cộng đồng
      </button>
      ...
      <div class="bg-white p-5 border rounded-lg shadow-sm">
        <div class="flex justify-between items-start">
          ...
          <div class="relative">
            <button @click="showMenu = !showMenu" class="text-gray-500 hover:text-gray-700 text-2xl font-bold px-2 -mt-2">
              ⋮
            </button>
            <div v-if="showMenu" class="absolute right-0 mt-2 w-48 bg-white border rounded-md shadow-lg z-10">
              <button v-if="isCreatorOfBuddyRead" @click="openInviteModal"
                class="block w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-yellow-50 rounded-t-md">
                Mời thành viên
              </button>
              
              <button v-if="isCreatorOfBuddyRead" @click="deleteBuddyRead" 
                :disabled="isDeleting"
                class="block w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-b-md disabled:opacity-50">
                {{ isDeleting ? 'Đang xóa...' : 'Xóa phòng đọc' }}
              </button>
              <button v-else @click="leaveBuddyRead" 
                :disabled="isDeleting"
                class="block w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-md disabled:opacity-50">
                {{ isDeleting ? 'Đang rời...' : 'Rời khỏi phòng đọc' }}
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-4 mt-1">
          <img 
            v-if="buddyRead.book?.cover_url"
            :src="`${COVER_IMAGE_SERVER_URL}/${buddyRead.book.cover_url}`" 
            :alt="buddyRead.book.title"
