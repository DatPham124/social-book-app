  ...
  <div v-else class="max-w-5xl mx-auto mt-8">
    ...
    <div v-else class="bg-white shadow rounded-xl p-6">
      ...
      <div class="flex items-start gap-6 border-b pb-4 relative">
        ...
        <div class="flex-1">
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="relative">
              <button @click="showMenu = !showMenu" class="text-gray-500 hover:text-gray-700 text-2xl font-bold px-2">⋮</button>
          ...
          <p class="text-gray-600 mt-1">
            Người quản lý: <span class="font-semibold text-yellow-600">{{ club.creator_name }}</span>
          </p>
      ...
      <div v-if="isMember" class="flex flex-wrap space-x-6 mt-4 border-b border-gray-200">
        <button v-for="tab in [{ key: 'upcoming', label: 'Cuộc họp' }, { key: 'discussions', label: 'Thảo luận' }, { key: 'members', label: 'Thành viên' }, { key: 'about', label: 'Giới thiệu' }, { key: 'rules', label: 'Nội quy' }]" 
          :key="tab.key" @click="activeTab = tab.key" class="py-2 text-sm font-medium transition-colors border-b-2"
          :class="activeTab === tab.key ? 'text-yellow-600 border-yellow-400' : 'text-gray-500 border-transparent hover:text-gray-700'">
          {{ tab.label }}
        </button>
      </div>
      <div v-else class="flex flex-wrap space-x-6 mt-4 border-b border-gray-200">
        <button v-for="tab in [{ key: 'about', label: 'Giới thiệu' }, { key: 'rules', label: 'Nội quy' }, { key: 'members', label: 'Thành viên' }]" 
          :key="tab.key" @click="defaultTab = tab.key" class="py-2 text-sm font-medium transition-colors border-b-2"
          :class="defaultTab === tab.key ? 'text-yellow-600 border-yellow-400' : 'text-gray-500 border-transparent hover:text-gray-700'">
          {{ tab.label }}
        </button>
      </div>
      ...
      <div v-if="isMember" class="mt-6">
        <div v-if="activeTab === 'upcoming'">
          ...
          <div v-else>
            <div v-if="meetings.filter(m => m.status === 'upcoming').length === 0" class="text-center py-10 text-gray-500">
              <p class="font-semibold italic mb-3">Chưa có cuộc họp sắp tới</p>
              <button v-if="isCreator" @click="showAddMeeting = true" class="px-4 py-2 bg-yellow-400 hover:bg-yellow-500 text-black font-semibold rounded-md transition">+ Tạo cuộc họp mới</button>
            </div>
            <div v-else class="space-y-4">
              <div v-for="meeting in meetings.filter(m => m.status === 'upcoming')" :key="meeting.id" class="p-4 border rounded-lg flex justify-between items-center hover:bg-gray-50 transition">
                <div>
                  <p class="font-semibold text-gray-800">{{ meeting.title }}</p>
                  <p class="text-sm text-gray-500">Ngày: {{ new Date(meeting.date).toLocaleDateString('vi-VN') }}, Giờ: {{ new Date(meeting.date).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }) }}</p>
                </div>
                <button @click="goToMeeting(meeting.id)" class="px-3 py-1 bg-yellow-400 hover:bg-yellow-500 rounded-md text-black text-sm font-medium">Chi tiết</button>
              </div>
              <div v-if="isCreator" class="flex justify-center mt-6">
                <button @click="showAddMeeting = true" class="px-4 py-2 bg-yellow-400 hover:bg-yellow-500 text-black font-semibold rounded-md transition">+ Thêm cuộc họp</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'discussions'">
          ...
          <div v-else>
            <div v-if="isCreator" class="flex justify-end mb-4">
              <button @click="showDiscussionForm = true" class="px-4 py-2 bg-yellow-400 hover:bg-yellow-500 text-black font-semibold rounded-md transition">+ Tạo thảo luận mới</button>
            </div>
            <div v-if="isLoadingDiscussions" class="text-center py-10 text-gray-500">Đang tải thảo luận...</div>
            <div v-else-if="discussionsList.length === 0" class="text-center py-16 text-gray-500 bg-gray-50 rounded-lg border">
              <span class="text-6xl">💬</span>
              <h3 class="text-xl font-semibold mt-4 text-gray-800">Chưa có thảo luận</h3>
              <p class="mt-1 text-gray-600">{{ isCreator ? 'Hãy tạo một chủ đề để mọi người cùng trao đổi!' : 'Chưa có chủ đề nào được tạo.' }}</p>
            </div>
            <div v-else class="space-y-4">
              <router-link v-for="post in discussionsList" :key="post.id" :to="'/discussion/' + post.id" class="block p-4 border rounded-lg bg-white shadow-sm hover:shadow-md transition">
                <p class="text-sm text-gray-500">Đăng bởi <span class="font-medium text-gray-700">{{ post.user?.username }}</span><span class="ml-2">&bull; {{ new Date(post.created_at).toLocaleDateString('vi-VN') }}</span></p>
                <h3 class="text-lg font-semibold text-gray-800 mt-1">{{ post.title }}</h3>
                <p class="text-gray-600 mt-1 truncate">{{ post.content }}</p>
                <p class="text-sm text-yellow-600 font-medium mt-2">{{ post.comment_count }} bình luận &rarr;</p>
              </router-link>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'members'">
          <div v-if="isCreator" class="mb-8 p-4 bg-gray-50 rounded-lg border">
            ...
            <input v-model="searchQuery" @input="searchUsers" type="text" class="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-yellow-400 focus:outline-none" placeholder="Tìm theo username..." />
            ...
            <ul v-if="searchResults.length > 0" class="mt-4 max-h-60 overflow-y-auto space-y-2">
              <li v-for="user in searchResults" :key="user.id" class="flex justify-between items-center p-2 border rounded-md bg-white hover:bg-gray-100">
          ...
          <div v-if="isCreator && joinRequests.length > 0" class="mb-8">
            <h3 class="text-lg font-semibold text-yellow-600 mb-3">Yêu cầu đang chờ duyệt ({{ joinRequests.length }})</h3>
            <div class="space-y-3">
              <div v-for="req in joinRequests" :key="req.id" class="flex items-center justify-between p-3 bg-white border rounded-lg shadow-sm">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center font-bold text-gray-700 overflow-hidden shadow-sm">
                    <img v-if="req.sender_profile?.avatar_url" :src="`${AVATAR_SERVER_URL}/${req.sender_profile.avatar_url}`" class="w-full h-full object-cover rounded-full" />
                ...
                <div class="flex gap-2">
                  <button @click="handleJoinRequest(req.id, 'accept')" class="px-3 py-1 bg-yellow-400 hover:bg-yellow-500 text-black text-sm font-semibold rounded-md">Chấp thuận</button>
                  <button @click="handleJoinRequest(req.id, 'decline')" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-md">Từ chối</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <router-link v-for="member in membersList" :key="member.user_id" :to="'/profile/' + member.user_id" class="flex items-center gap-3 p-3 bg-white border rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                <div class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center font-bold text-gray-700 overflow-hidden shadow-sm flex-shrink-0">
                  <template v-if="member.avatar_url"><img :src="`${AVATAR_SERVER_URL}/${member.avatar_url}`" alt="Avatar" class="w-full h-full object-cover" /></template>
        ...
        <div v-else-if="activeTab === 'about' || activeTab === 'rules'" class="bg-gray-50 p-4 sm:p-5 rounded-lg border">
          <p v-if="activeTab === 'about'" class="whitespace-pre-line leading-relaxed text-gray-800">{{ club.description || "Chưa có mô tả." }}</p>
          <div v-if="activeTab === 'rules' && rulesList.length > 0">
            <ol class="list-decimal list-outside pl-5 space-y-2 text-gray-800 leading-relaxed"><li v-for="(rule, index) in rulesList" :key="index">{{ rule }}</li></ol>
          </div>
          <p v-if="activeTab === 'rules' && rulesList.length === 0" class="text-center text-gray-500 italic">Chưa có nội quy.</p>
        </div>
      </div>
      
      <!-- Non-member View -->
      <div v-else class="mt-6">
        <div v-if="defaultTab === 'about' || defaultTab === 'rules'" class="bg-gray-50 p-4 sm:p-5 rounded-lg border">
          <p v-if="defaultTab === 'about'" class="whitespace-pre-line leading-relaxed text-gray-800">{{ club.description || "Chưa có mô tả." }}</p>
          <div v-if="defaultTab === 'rules' && rulesList.length > 0">
            <ol class="list-decimal list-outside pl-5 space-y-2 text-gray-800 leading-relaxed"><li v-for="(rule, index) in rulesList" :key="index">{{ rule }}</li></ol>
          </div>
          <p v-if="defaultTab === 'rules' && rulesList.length === 0" class="text-center text-gray-500 italic">Chưa có nội quy.</p>
        </div>
